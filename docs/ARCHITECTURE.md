...
| Read-only backend assertions (Mongo, read-only by construction) + manual one-time login (no secret needed, human logs in, session persists) | `browser/db.py::ReadOnlyCollection`, `stages/manual_login.py::manual_login` |
