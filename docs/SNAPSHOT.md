...
- F-008 **Video ingest: Gemini vision provider + stages/ingest.py video-to-FlowSpec mapping** [high] — Watches a demo video via GeminiProvider.see_video and maps the VideoObservation into a fresh FlowSpec (screens/flows/steps), every Step provenance-tracked to the source video's t_start/t_end. Tested end-to-end against MockProvider only (6 tests in tests/test_ingest.py + 4 GeminiProvider tests in tests/test_providers.py, all green). · reason: T-060 note names a further bar this unit does not attempt: >=90% step recall vs a hand-written list on a real demo video. No Pathlynks demo video exists in this repo yet, so that golden-test acceptance is still undeliverable -- closed T-060 on its literal done_check (uv run pytest tests/test_ingest.py -q, exit 0) only; the recall bar is a follow-on task once a real video lands.
...
- T-110 [high] Regression proof: break a staging feature, confirm exactly that case FAILs
